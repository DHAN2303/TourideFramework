...
namespace Touride.Framework.Data.Extensions
{
    /// <summary>
    ...
    public static class PagedListExtensions
    {
        ...
        public static async Task<IPagedList<T>> ToPagedListAsync<T>(this IQueryable<T> source, int pageNumber, int pageSize, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (pageNumber <= 0)
            {
        ...
        public static async Task<IPagedList<T>> ToPagedListWithNoLockAsync<T>(this IQueryable<T> source, int pageNumber, int pageSize, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (pageNumber <= 0)
            {
                throw new ArgumentException($"pageNumber: {pageNumber} <= 0 , pageNumber > 0 olmalı.");
